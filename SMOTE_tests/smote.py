...
failed_loans = con.execute(
    "SELECT amount, account_id FROM loan_dev WHERE status == -1").fetchall()
...
for account in accounts:
    ...
    normalized_accounts.append([account[0], account[1], account[2], account[3],
                                getNormalizedAverageTransAmount(
                                    account[4]), getNormalizedAverageBalance(account[5]),
                                getNormalizedMinBalance(
                                    account[6]), getNormalizedMaxBalance(account[7]),
...
for i in range(len(failed_loans)):
    ...
    normalizedValues.append([normalized_failed_loans[i][0], normAccount[4], normAccount[5],
                            normAccount[6], normAccount[7], normAccount[8], normAccount[9]])
...
for i in range(loans_to_generate):
    ...
    values = [basis_loan[0], basis_acc[4], basis_acc[5],
              basis_acc[6], basis_acc[7], basis_acc[8], basis_acc[9]]
    target_loan = getNearestNeighbours(values, normalizedValues, k)[k-1]
    newValues = []
    for j in range(len(values)):
        newValues.append(
            values[j] + (normalizedValues[target_loan][j] - values[j]) * delta)
    new_loan_id = 8000 + i
    new_account_id = 4501 + i
    new_loan_month = randint(0, 12)
    new_loan_date = str(getRandomYear()) + "-" + "{:02d}".format(
        new_loan_month) + "-" + "{:02d}".format(getRandomDay(new_loan_month))
    new_loan_duration = getRandomDuration()
    new_loan_status = -1
    new_loan_payments = getDenormalizedLoanAmount(
        newValues[0])/new_loan_duration
    new_account_month = randint(1, 11)
    new_account_date = str(getRandomYear()) + "-" + "{:02d}".format(
        new_account_month) + "-" + "{:02d}".format(getRandomDay(new_account_month))
    q = "INSERT INTO loan_dev (loan_id, account_id, date, amount, duration, payments, status) VALUES (" + str(new_loan_id) + ", " + str(new_account_id) + ", '" + new_loan_date + \
        "', " + str(getDenormalizedLoanAmount(newValues[0])) + ", " + str(
            new_loan_duration) + ", " + str(new_loan_payments) + ", " + str(new_loan_status) + ")"
    print(q)
    con.execute(q)
    q = "INSERT INTO account (account_id, district_id, frequency, date, average_trans_amount, average_balance, min_balance, max_balance, min_transaction_amount, max_transaction_amount) VALUES (" + str(new_account_id) + ", " + str(getRandomDistrict()) + ", '" + getRandomFrequency() + "', '" + new_account_date + "', " + str(
        getDenormalizedAverageTransAmount(newValues[1])) + ", " + str(getDenormalizedAverageBalance(newValues[2])) + ", " + str(getDenormalizedMinBalance(newValues[3])) + ", " + str(getDenormalizedMaxBalance(newValues[4])) + ", " + str(getDenormalizedMinTransAmount(newValues[5])) + ", " + str(getDenormalizedMaxTransAmount(newValues[6])) + ")"
    print(q)
con.commit()
