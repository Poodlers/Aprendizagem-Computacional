...
def make_into_date(var1):
    ...
    return datetime.strptime(var[4:6] + "-" + var[2:4] + '-' + var[0:2], '%d-%m-%y').date()
...
gender = ["M" if int(str(client_df['birth_number'][i])[2:4]) <= 12 else "F" for i in range(client_df['birth_number'].size)] 
new_client_df = client_df.assign(gender=gender)

new_client_df['birth_number'] = new_client_df['birth_number'].apply(make_into_date)
...
new_client_df.to_sql('client', con=conn, if_exists='replace', index=False, index_label='client_id')
